					...
					<tr><td>
						...
						<td>
							<c:if test="${orderList.pickstatus == 0}">
								<a 
								href="/demo/picksub.jsp?orderitemid=${orderList.orderitemid}&quantity=${orderList.quantity}&weight=${orderList.weight}&orderimg=${orderList.orderimg}">取货</a>
							</c:if>
		...
		<div style="float:left;width:100%;text-align:center">
				<ul class="pagination">
					<c:if test="${requestScope.pager.hasFirst==true}">
						<li><a href="getPickOrderList.action?currentPage=1">首页</a></li>
					</c:if>
					...
					<c:if test="${requestScope.pager.hasPrevious==true}">
						<li><a
							href="getPickOrderList.action?currentPage=${requestScope.pager.currentPage-1}">
								上一页 </a></li>
					</c:if>
					...
					<c:if test="${requestScope.pager.hasLast==true}">
						<li><a
							href="getPickOrderList.action?currentPage=${requestScope.pager.totalPage}">
								尾页 </a></li>
					</c:if>
					<c:if test="${requestScope.pager.hasNext==false}">
						<li class="disabled"><a
